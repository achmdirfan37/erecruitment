...
class PelamarList_InvitationMCU extends React.Component {
  constructor() {
    super()
  ...
  componentDidMount() {
    ...
    //   })
    // })
    axios.get('http://127.0.0.1:8000/api/tr_lamaran_kerja/viewMCU')
      .then(response => {
        this.setState({
  ...
  handlePageChange(pageNumber) {
    ...
    axios.get('http://127.0.0.1:8000/api/tr_lamaran_kerja/viewMCU?page=' + pageNumber)
      .then(response => {
        this.setState({
          tr_lamaran_kerja: response.data.data,
          itemsCountPerPage: response.data.per_page,
                        ...
                        this.state.tr_lamaran_kerja !== undefined
                          ? this.state.tr_lamaran_kerja.map(tr_lamaran_kerjas => (
                            <tr key={tr_lamaran_kerjas.id}>
                              ...
                              <td>{tr_lamaran_kerjas.lk_sta_kon_mcu}</td>
                              <td>{tr_lamaran_kerjas.lk_ket_mcu}</td>
                              <td>
                                <Link
                                  to={`/${tr_lamaran_kerjas.id}/Undang_MCU`}
                                  className="btn btn-warning btn-sm mr-2">Konfirmasi Undangan MCU</Link>
                              </td>
                            </tr>
                          ))
                          :
...
export default PelamarList_InvitationMCU;