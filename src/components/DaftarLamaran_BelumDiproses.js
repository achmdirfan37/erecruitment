...
class DaftarLamaran_BelumDiproses extends React.Component {
  constructor() {
    super()
  ...
  componentDidMount() {
    ...
    axios.get(`http://127.0.0.1:8000/api/tr_lamaran_kerja/viewStatusBelumDiproses/${id}`)
      .then(response => {
        this.setState({
  ...
  handlePageChange(pageNumber) {
    ...
    axios.get(`http://127.0.0.1:8000/api/tr_lamaran_kerja/viewStatusBelumDiproses/${id}?page=` + pageNumber)
      .then(response => {
        this.setState({
          tr_lamaran_kerja: response.data.data,
          itemsCountPerPage: response.data.per_page,
  ...
  cari = () => {
    const id = this.props.match.params.id;
    axios.get(`http://localhost:8000/api/tr_lamaran_kerja/searchViewStatusBelumDiproses/${id}?cari=` + this.state.cari)
      .then(response => {
        this.setState({
          tr_lamaran_kerja: response.data.data,
          itemsCountPerPage: response.data.per_page,
  ...
  pindahTerpilih(id) {
    axios.delete('http://127.0.0.1:8000/api/tr_lamaran_kerja/ubahStatusTerpilih/' + id)
      .then(response => {
        var tr_lamaran_kerjass = this.state.tr_lamaran_kerja;
        for (var i = 0; i < tr_lamaran_kerjass.length; i++) {
          if (tr_lamaran_kerjass[i].id == id) {
                            ...
                            <tr key={tr_lamaran_kerjas.id}>
                              ...
                              <td>
                                <Link
                                  ...
                                  onClick={this.pindahTerpilih.bind(
                                    this,
...
export default DaftarLamaran_BelumDiproses;