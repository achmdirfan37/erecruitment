...
class DaftarLamaran_Placement extends React.Component {
  constructor() {
    super()
  ...
  componentDidMount() {
    axios.get('http://127.0.0.1:8000/api/tr_lamaran_kerja/viewStatusPlacement')
      .then(response => {
        this.setState({
  ...
  handlePageChange(pageNumber) {
    ...
    axios.get('http://127.0.0.1:8000/api/tr_lamaran_kerja/viewStatusPlacement?page=' + pageNumber)
      .then(response => {
        this.setState({
          tr_lamaran_kerja: response.data.data,
          itemsCountPerPage: response.data.per_page,
  ...
  cari = () => {
    axios.get('http://localhost:8000/api/tr_lamaran_kerja/searchViewStatusPlacement?cari=' + this.state.cari)
      .then(response => {
        this.setState({
          tr_lamaran_kerja: response.data.data,
          itemsCountPerPage: response.data.per_page,
                        ...
                        this.state.tr_lamaran_kerja !== undefined
                          ? this.state.tr_lamaran_kerja.map(tr_lamaran_kerjas => (
                            <tr key={tr_lamaran_kerjas.id}>
                              ...
                              <td>
                                <Link to={`/${tr_lamaran_kerjas.id}/DetailLamaran_Pelamar`} className="btn btn-warning btn-sm mr-2">Lihat Detail Informasi</Link>
                              </td>
                            </tr>
                          ))
                          :
...
export default DaftarLamaran_Placement;