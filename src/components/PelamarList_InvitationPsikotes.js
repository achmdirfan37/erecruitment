...
class PelamarList_InvitationPsikotes extends React.Component {
  constructor() {
    super()
  ...
  componentDidMount() {
    ...
    //   })
    // })
    axios.get('http://127.0.0.1:8000/api/tr_lamaran_kerja/viewPsikotes')
      .then(response => {
        this.setState({
  ...
  handlePageChange(pageNumber) {
    ...
    axios.get('http://127.0.0.1:8000/api/tr_lamaran_kerja/viewPsikotes?page=' + pageNumber)
      .then(response => {
        this.setState({
          tr_lamaran_kerja: response.data.data,
          itemsCountPerPage: response.data.per_page,
                  ...
                  <table id="example1" className="table table-bordered table-striped">
                    <thead>
                      <tr>
                        ...
                        <th>Konfirmasi Persetujuan</th>
                        <th>Keterangan Menolak</th>
                        <th>Action</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        this.state.tr_lamaran_kerja !== undefined
                          ? this.state.tr_lamaran_kerja.map(tr_lamaran_kerjas => (
                            <tr key={tr_lamaran_kerjas.id}>
                              ...
                              <td>{tr_lamaran_kerjas.lk_sta_kon_psikotes}</td>
                              <td>{tr_lamaran_kerjas.lk_ket_psikotes}</td>
                              <td>
                                <Link
                                  to={`/${tr_lamaran_kerjas.id}/Undang_Psikotes`}
                                  className="btn btn-warning btn-sm mr-2">Konfirmasi Undangan Psikotes</Link>
                              </td>
                            </tr>
                          ))
                          :
...
export default PelamarList_InvitationPsikotes;