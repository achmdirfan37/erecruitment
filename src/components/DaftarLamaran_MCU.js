...
class DaftarLamaran_MCU extends React.Component {
  constructor() {
    super()
  ...
  componentDidMount() {
    ...
    axios.get(`http://127.0.0.1:8000/api/tr_lamaran_kerja/viewStatusMCU/${id}`)
      .then(response => {
        this.setState({
  ...
  handlePageChange(pageNumber) {
    ...
    axios.get(`http://127.0.0.1:8000/api/tr_lamaran_kerja/viewStatusMCU/${id}?page=` + pageNumber)
      .then(response => {
        this.setState({
          tr_lamaran_kerja: response.data.data,
          itemsCountPerPage: response.data.per_page,
  ...
  cari = () => {
    const id = this.props.match.params.id;
    axios.get(`http://localhost:8000/api/tr_lamaran_kerja/searchViewStatusMCU/${id}?cari=` + this.state.cari)
      .then(response => {
        this.setState({
          tr_lamaran_kerja: response.data.data,
          itemsCountPerPage: response.data.per_page,
  ...
  pindahPlacement(id) {
    axios.delete('http://127.0.0.1:8000/api/tr_lamaran_kerja/ubahStatusPlacement/' + id)
      .then(response => {
        var tr_lamaran_kerjass = this.state.tr_lamaran_kerja;
        for (var i = 0; i < tr_lamaran_kerjass.length; i++) {
          if (tr_lamaran_kerjass[i].id == id) {
                          ...
                          ? this.state.tr_lamaran_kerja.map(tr_lamaran_kerjas => (
                            <tr key={tr_lamaran_kerjas.id}>
                              ...
                              <td>
                                ...
                                <Link
                                  className="btn btn-warning btn-sm mr-2"
                                  onClick={this.pindahPlacement.bind(
                                    this,
                                    tr_lamaran_kerjas.id
                                  )}
                                >
                                ...
                                <Link to={`/${tr_lamaran_kerjas.id}/Undang_MCU`} className="btn btn-warning btn-sm mr-2">Undang MCU</Link>
                              </td>
                            </tr>
                          ))
                          :
...
export default DaftarLamaran_MCU;